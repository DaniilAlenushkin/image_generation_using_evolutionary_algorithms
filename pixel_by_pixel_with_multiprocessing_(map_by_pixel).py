...
def evolve_pixel(target_pixel_component):
    POPULATION_SIZE = 100
    ...
    for generation in range(NUMBER_OF_GENERATIONS):
        norm_vector = fitness(population, target_pixel_component)
    ...
    norm_vector = fitness(population, target_pixel_component)
    best_pixel = np.array(population[norm_vector.index(max(norm_vector))])
    return best_pixel
...
if __name__ == '__main__':
    ...
    for h in range(1, height+1):
        ...
        for w in range(1, width+1):
            target_pixel_components = [target_array[h-1:h, w-1:w,:][0][0][c] for c in range(channels)]
            with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
                pixel_result = pool.map(evolve_pixel, target_pixel_components)
            row_result.append(np.array(pixel_result))
            print(h, w, 'DONE')
